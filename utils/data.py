    ...
    def __init__(self, root, patch_size=320, download=True, validation=False, transform=None):
        if download:
          if not os.path.isdir(root):
            ...
            with tarfile.open(root+"/imagenette2-320.tgz", 'r:gz') as tar:
                # Extract all contents to the specified directory
                tar.extractall(path=root)
        
        self.folder = Path(root+'/imagenette2-320/train') if not validation else Path(root+'/imagenette2-320/val')
